...
function countApplesAndOranges(s, t, a, b, apples, oranges) {
    ...
    let numApple = 0, numOrange = 0
    let locationApple = apples.map(item => item + a)
    let locationOrange = oranges.map(item => item + b)
    
    locationApple.forEach(element => {
        (s <= element && element <= t) ? numApple++ : numApple
    })
    console.log(numApple)
    
    locationOrange.forEach(element => {
        (s <= element && element <= t) ? numOrange++ : numOrange
    })
